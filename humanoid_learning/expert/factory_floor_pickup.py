...
class FactoryFloorPickup:
    ...
    def step(self):
        ...
        if self.stage in ('CLOSE', 'PICK_CLEAR'):
            ...
            self.last_q = self.posture.solve(height, palms, rotations, pitch, iterations=8,
    ...
    def _pick_clear_action(self):
        """Lift slightly forward/up with arms before moving the loaded body."""
        m, d = self.env.model, self.env.data
        action = np.zeros(14)
        translation = self.recovery.expert.task_rotation[:,0]*.00004 + np.array([0.,0.,.00008])
        for i, side in enumerate(('left','right')):
            ...
            dq = jac.T @ np.linalg.solve(jac@jac.T+1e-4*np.eye(6),delta)
    ...
    def _contact_action(self, forces, hold_legs=True):
        ...
        for i, side in enumerate(('left', 'right')):
            ...
            if load > 1.5 or (not self.side_entry_ready[side]
                    and abs(local_point[2]-patch[2]) < .005
            ...
            rotation_error = orientation_error(self.env.palm_pose(side)[1], self.close_hand_rotations[i])
    ...
    def _side_face(self, side):
        ...
        axis = int(np.argmax(np.abs(rotation[:,:2].T @ outward)))
